...
def combine_src(text_path, save_path):
        ...
        while True:
            ...
            if (index + 1) < num_lines:
                ...
                if next_spk == spk and topic_idx == next_topic_idx:
                    #读文件，文本
                    test_str = re.search(r"\W", tmp[-2])
                    if test_str != None:
                        text_list.append(tmp[-2])
                    else:
                        text_list.append(tmp[-2] + "，")
                    ...
                    print(" ".join([wav_name + "_" + tp, save_file, text, label]))
                    index += 1 
                    break
            else:
                index += 1
    ...
    # if next_spk == spk and topic_idx == next_topic_idx:
        ...
        print(" ".join([tmp[0], save_file, tmp[-2], tmp[-1]]))


if __name__ =="__main__":
    import sys
    if len(sys.argv) != 3:
        print("python combine_wav.py src_path combine_src_save_path")
        sys.exit()
