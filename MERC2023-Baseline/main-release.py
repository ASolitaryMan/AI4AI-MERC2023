...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        if args.model_type == 'mlp':
            features, emos_out, val_out = model(multi_feat)
        elif args.model_type == 'attention':
            features, emos_out, val_out = model(audio_feat, text_feat, visual_feat)
...
if __name__ == '__main__':
    ...
    for ii in range(args.num_folder):
        ...
        for epoch in range(args.epochs):
            ...
            eval_metric = overall_metric(eval_results['emo_fscore']) # bigger -> better
            ...
            print ('epoch:%d; train_fscore:%.4f; eval_fscore:%.4f' %(epoch+1, train_results['emo_fscore'], eval_metric))

            # if max_eval_metric < eval_metric:
            #     max_eval_metric = eval_metric
            #     save_path = "/home/wc/workspace/MER2023-Baseline/model/" + str(ii) + "_model.pt"
        ...
        folder_evalres.append([best_evalfscore])
        end_time = time.time()
    ...
    ## analyze cv results
    cv_fscore = np.mean(np.array(folder_evalres), axis=0).item()
    res_name = f'f1:{cv_fscore:.4f}'
    ...
    for setname in args.test_sets:
        ...
        if os.path.exists(label_path):
            if setname in ['test3']:          emo_fscore, final_metric = report_results_on_test3(label_path, pred_path)
